...
# update distance matrix between node i and j
    ...
    # consider if necessary, may leave out as a new function
...
def update_row(bdist,blockFlag,i,y):
    ...
    #y = np.zeros(1,nb*bs, dtype=constants.DATA_TYPE);
    bi,ii=constants.getbi(i)
    k=0
    for bj in range(0,bi):
        if blockFlag[bj]:
...
def gen_pointers3(distVec, nodeFlag, mi, prevVec, nextVec, rowind, beditPrev, beditNext):
    # mi: index in the big matrix
    valinds = mi+1+np.where(nodeFlag[mi+1:])[0]
