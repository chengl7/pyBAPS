...
def run_localserver(n, d, ls_id, df, bf, nb):
    ls.LocalServer(n,d,ls_id,df,bf,nb)


def test_all_iterations(n):
    ...
    for i in range(n):
        ...
        n_blocks = 1
        p = Pool(1)
        p = Process(target=run_localserver, args=(n, d, 0, 'tests/test_data', 'tests/test_block_files', n_blocks))
        p.start()
