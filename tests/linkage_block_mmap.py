...
import numpy as np
from common_base import BlockFileMap
from common_base import init_files
...
def linkage_block(X, base_directory):
    ...
    for bi in range(0,constants.N_BLOCK):
        for bj in range(bi,constants.N_BLOCK):
            ...
            bfn = base_directory+"/{}_n/{}_n.block".format(bi, bj)
            bfp = base_directory+"/{}_p/{}_p.block".format(bi, bj)
            bnext[bi, bj] = BlockFileMap(bfn, constants.DATA_TYPE, (constants.BLOCK_SIZE, constants.BLOCK_SIZE))
            bprev[bi, bj] = BlockFileMap(bfp, constants.DATA_TYPE, (constants.BLOCK_SIZE, constants.BLOCK_SIZE))
            bnext[bi,bj].open()
            bprev[bi,bj].open()
