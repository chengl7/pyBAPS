...
def make_folders(base_directory, nb):
...
class BlockFileMap():
    ...
    def __init__(self, path, arr, dtype):
        """ Initialized with data, creates a file, writes to the file """
        self.file_path = path
        self.row_size = arr.shape[0]
        self.dtype = dtype
        self.mmap = None
        # Create a file 
    ...
    def _mmap(self):
        ...
        self.mmap = np.memmap(self.file_path, dtype=self.dtype, mode='r+', shape=(self.row_size, self.row_size), order='C')
