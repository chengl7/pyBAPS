...
logging.basicConfig(level=logging.WARNING, format='%(asctime)s - %(processName)s - %(levelname)s - %(message)s')
...
#        logger.info('%dth step, merge index-node %d-%d and %d-%d.\n' % (iStep,ii,treeNodeArr[ii],jj,treeNodeArr[jj]))
        logger.warn('%dth step, merge index-node %d-%d and %d-%d.\n' % (iStep,ii,treeNodeArr[ii],jj,treeNodeArr[jj]))
...
#    globalServer.check_child_conns()  # ensure child connections are ready
    ...
    # compare with standard linkage algorithm
    X = Constants.get_input_data()
    from scipy.cluster.hierarchy import linkage
    Z1 = linkage(X, 'complete')
    if np.all(Z1[:,:2]-Z[:,:2]<1e-3):
        print('generated the same result')
    else:
        print(Z)
        print(Z1)
        print(Z1[:,:2]-Z[:,:2])
